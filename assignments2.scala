object assignments2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    peter.greeting(new PersonTask3("Yubin", "Zhong"))
    peter.greeting(new PersonTask3("David", "Beckham"))
    peter.greeting(new PersonTask3("Michael", "Jackson"))
    println("Task4:")
    println(task4(x => x * x, 2))
    println("Task5:")
    object p1 extends PersonTask5("Yubin", "Zhong") with Employee
    p1.salary_(1000)
    p1.taxToPay
    object p2 extends PersonTask5("Yubin", "Zhong") with Student
    p2.taxToPay
    object p3 extends PersonTask5("Yubin", "Zhong") with Teacher
    p3.salary_(1000)
    p3.taxToPay
    object p4 extends PersonTask5("Yubin", "Zhong") with Employee with Student
    p4.salary_(1000)
    p4.taxToPay
    object p5 extends PersonTask5("Yubin", "Zhong") with Student with Employee
    p5.salary_(1000)
    p5.taxToPay
  }
...
case class PersonTask3(var firstName: String, var lastName: String) {
  def greeting(person: PersonTask3): Unit = person match {
    ...
    case PersonTask3(_, "Zhong") => println("Hi! Mr. Zhong")
